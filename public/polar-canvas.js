...
// Copied from signalk-polar-performance-plugin's viewer (offscreen grid/curve
// cache; live-overlay methods are kept but unused here — this plugin has no
// live tracking, it only displays the stored canonical curves).
//
...
(function (global) {
  ...
  // Display units are read from optional /meta-shaped data so the server's user
  // preferences (kn vs m/s, ° vs rad) are respected; defaults to kn/degrees.
  // ---------------------------------------------------------------------------
  ...
  function PolarCanvas(canvas, opts) {
    ...
    // Live data (updated every poll cycle) — unused by this plugin
    ...
    // Display unit metadata, shaped like GET /meta from the performance plugin
    this._meta         = null
   ...
   * Replace all library curve data and rebuild the offscreen cache.
    * @param {number[]} twsList   - TWS values in m/s
    * @param {Object}   curves    - twsMs → curve object
