...
class PolarStore {
  ...
  save(id, resource) {
    ...
    // The storage key is the only authoritative id; never persist a caller-supplied copy.
    const { id: _ignored, ...body } = resource
    fs.writeFileSync(this._jsonPath(id), JSON.stringify(body, null, 2), 'utf8')
    return id
  }
