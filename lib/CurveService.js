...
function sampleCurve(polar, tws, stepRad, performanceFactor) {
  ...
  const sideTarget = (target) => (target && Number.isFinite(target.twa) && Number.isFinite(target.speed))
    ? { twa: target.twa, tbs: target.speed }
    : null
  return {
    tws,
    points,
    beat: sideTarget(targets?.beat),
    run: sideTarget(targets?.run)
  }
}
...
function buildCurves(table, stepDeg = DEFAULT_STEP_DEG, performanceFactor = 1) {
  ...
  const curves = table.axes.tws
    .map(tws => {
      try {
        return sampleCurve(polar, tws, stepRad, performanceFactor)
      } catch {
        return null // one unsampleable TWS row must not abort the whole response
      }
    })
    .filter(Boolean)
